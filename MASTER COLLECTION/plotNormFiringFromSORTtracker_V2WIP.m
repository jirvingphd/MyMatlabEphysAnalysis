...
responseFields=fieldnames(SORTtracker);
for r = 1:length(responseFields)
    ...
    %     dataConcatToPlot=vertcat(SORTtracker.(currResponse).sortedData.dataSplit.dataSplitSortedMat);
    %     dataConcatToPlotUnflipd=dataConcatToPlot;
    %     dataSortedToPlot=flipud(SORTtracker.(currResponse).sortedData.dataToPlot_blSubdSorted);
    %     idxQU_ConcatToPlot=vertcat(SORTtracker.(currResponse).sortedData.rdataSplit.idxQU_Split);
    
    
    %     SORTtracker.(responseFields{r}).sortedData.dataReadyToPlot=dataSortedToPlot;
    ...
    blSubDataToPlot=SORTtracker.(currResponse).sortedData.sortedRateDataToPlot;
